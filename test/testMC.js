...
const root =
  "0xb8851699b8e764dc99a286fdbc20411bec24a4eb54373cf8e40ce88d60382fc6";
...
describe("TestMC", function () {
  ...
  describe("Giveaways", function () {
    it("Should mint 99 giveaways", async function () {
      await DContract.mintMultipleByOwner(
        ethAddressList,
        [1, 2, 1, 1, 1, 1, 3, 2, 2]
      );
      expect(await (await DContract.totalSupply()).toString()).to.equal("9");
      expect(await (await DContract.categories(1))["counterSupply"]).to.equal(
        5
      );
      expect(await (await DContract.categories(2))["counterSupply"]).to.equal(
        3
      );
      expect(await (await DContract.categories(3))["counterSupply"]).to.equal(
        1
      );
  ...
  isPresale &&
    describe("Presale", function () {
      ...
      it("Should fail if Presale is not active", async function () {
        expect(
          DContract.connect(accounts[0]).presaleMint([1, 0, 0], proof, {
      ...
      it("Should fail if not Whitelisted", async function () {
        ...
        await expect(
          DContract.connect(accounts[1]).presaleMint([1, 0, 0], proof, {
      ...
      it("Should mint if Whitelisted", async function () {
        await DContract.setPresaleActive();
        expect(
          await DContract.connect(accounts[0]).presaleMint([2, 0, 0], proof, {
            value: await (price1 * 2).toString(),
          })
        );
      ...
      it("Should mint multiple differents NFT", async function () {
        await DContract.setPresaleActive();
        expect(
          await DContract.connect(accounts[0]).presaleMint([2, 1, 1], proof, {
        ...
        expect(
          await (
            await DContract.categories(1)
          )["counterWhitelistSupply"]
        ).to.equal(2);
        expect(
          await (
            await DContract.categories(2)
          )["counterWhitelistSupply"]
        ).to.equal(1);
        expect(
          await (
            await DContract.categories(3)
          )["counterWhitelistSupply"]
        ).to.equal(1);
        expect(await DContract.NFTcategory(0)).to.equal(1);
      ...
      it("Should fail if Whitelist supply is reached", async function () {
        await DContract.setPresaleActive();
        expect(
          await DContract.connect(accounts[0]).presaleMint([0, 0, 5], proof, {
  ...
  isPublicSale &&
    describe("Public Sale", async function () {
      ...
      it("Should fail if Presale is active", async function () {
        await DContract.setPresaleActive();
        await expect(
          DContract.connect(accounts[1]).mintNFT([1, 0, 0], {
            value: await (price1 * 2).toString(),
          })
      ...
      it("Should fail if Ether value sent is not correct", async function () {
        await expect(
          DContract.connect(accounts[1]).mintNFT([4, 0, 0], {
            value: await (price1 * 1).toString(),
          })
      ...
      it("Should mint 2 NFT", async function () {
        expect(
          await DContract.connect(accounts[1]).mintNFT([2, 0, 0], {
            value: await (price1 * 2).toString(),
          })
        );
      ...
      it("Should mint multiple differents NFT", async function () {
        expect(
          await DContract.connect(accounts[0]).mintNFT([2, 1, 1], {
            value: await (price1 * 2 + price2 * 1 + price3 * 1).toString(),
          })
        );
        expect(await DContract.ownerOf(0)).to.equal(accounts[0].address);
      ...
      it("Should fail if supply is reached", async function () {
        ...
        expect(
          await DContract.connect(accounts[0]).mintNFT([11, 0, 0], {
      ...
      it("Should withdraw the money", async function () {
        expect(
          await DContract.connect(accounts[1]).mintNFT([1, 0, 0], {
            value: await (price1 * 1).toString(),
          })
        );
        expect(
          await DContract.connect(accounts[2]).mintNFT([0, 1, 1], {
            value: await (price2 * 1 + price3 * 1).toString(),
          })
        );
        expect(
          await DContract.connect(accounts[3]).mintNFT([0, 2, 0], {
            value: await (price2 * 2).toString(),
          })
        );
        expect(
          await DContract.connect(accounts[4]).mintNFT([1, 0, 0], {
            value: await (price1 * 1).toString(),
          })
        );
        expect(
          await DContract.connect(accounts[5]).mintNFT([1, 0, 0], {
            value: await (price1 * 1).toString(),
          })
        );
        expect(
          await DContract.connect(accounts[6]).mintNFT([2, 1, 1], {
            value: await (price1 * 2 + price2 * 1 + price3 * 1).toString(),
          })
        );
        ...
        await expect(
          (await (await provider.getBalance(DContract.address)).toString()) /
            1000000000000000000
        ).to.equal(379);
  ...
  describe("change price", function () {
    it("Should change the NFT price", async function () {
      expect(await (await DContract.categories(1))["NFTPrice"]).to.equal(
    ...
    it("Should change multiple NFT supplies", async function () {
      ...
      expect(await (await DContract.categories(3))["NFTPrice"]).to.equal(
        ethers.utils.parseEther("98")
      );
      await DContract.changePrice([
        ethers.utils.parseEther("1"),
        0,
  ...
  describe("Token URI", function () {
    it("Should set the right URI", async function () {
      ...
      await expect(
        await DContract.connect(accounts[1]).mintNFT([0, 1, 1], {
          value: await (price2 * 1 + price3 * 1).toString(),
        })
      );
    ...
    it("Should fail for non existing token", async function () {
      await expect(DContract.tokenURI(0)).to.be.revertedWith(
